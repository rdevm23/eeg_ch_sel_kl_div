...
for i = 1: n_subs
     ...
     cz_sig = reshape(signals(8,:,:),[n_trials,n_samples]);
     c3_sig = reshape(signals(10,:,:),[n_trials,n_samples]);
     c4_sig = reshape(signals(12,:,:),[n_trials,n_samples]);
     ...
     disp("KLDs are Evaluated for one subject")
end
